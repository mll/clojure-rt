...
Class* Class_create(String *name, String *typeName, uint64_t fieldCount, ...) {
  ...
  Object *super = allocate(sizeof(Object) + sizeof(Class) + sizeof(uint64_t *) + fieldCount * sizeof(String *));
...
int64_t Class_fieldIndex(Class *self, String *field) {
  uint64_t initialGuess = String_hash(field) % self->fieldCount;
  if (String_equals(field, self->fields[initialGuess])) {
    release(self);
    release(field);
    return initialGuess;
  }
