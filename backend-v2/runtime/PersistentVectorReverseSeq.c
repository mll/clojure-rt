...
RTValue PersistentVectorReverseSeq_create(PersistentVector *v, int32_t index) {
    if (index < 0) return RT_boxNil();
    PersistentVectorReverseSeq *self = (PersistentVectorReverseSeq *)allocate(sizeof(PersistentVectorReverseSeq));
...
RTValue PersistentVectorReverseSeq_cons(PersistentVectorReverseSeq *self, RTValue item) {
    Ptr_release(self);
    release(item);
    throwIllegalArgumentException_C("cons not yet implemented for PersistentVectorReverseSeq");
}
