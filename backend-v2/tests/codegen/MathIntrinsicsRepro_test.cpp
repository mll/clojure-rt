...
static void test_math_pow_uaf(void **state) {
  ...
  ASSERT_MEMORY_ALL_BALANCED({
    ...
    try {
      ...
      std::cout << "Caught expected LanguageException: " << e.what()
                << std::endl;
    } catch (const std::exception &e) {
      std::cout << "Caught expected std::exception: " << e.what() << std::endl;
    }

    std::cout << "Invalidating module, this EXPECTED TO CRASH if UAF exists..."
              << std::endl;
    engine.retireModule("repro_uaf");
...
static void test_math_pow_leak(void **state) {
  ...
  ASSERT_MEMORY_ALL_BALANCED({
    ...
    engine.retireModule("repro_leak");
  });
}
