    ...
    if (method.closedovers_size() > 0) {        
      ...
      for(int i=0; i<method.closedovers_size(); i++) {
        ...
        if(functionArgTypes.find(name) == functionArgTypes.end()) {
          // Prepare indices for GEP: first index 0, second index i.
          auto zeroIndex = ConstantInt::get(*TheContext, APInt(64, 0));
          auto elementIndex = ConstantInt::get(*TheContext, APInt(64, i));
          
          // Create a vector (or array) of indices
          std::vector<Value*> indices = { zeroIndex, elementIndex };
          
          // Use the two indices in CreateGEP
