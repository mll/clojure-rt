...
static void setup_repro_compiler_state(rt::ThreadsafeCompilerState &compState) {
  ...
  fnDesc.argTypes = {ObjectTypeSet::dynamicType(),
...
static void test_double_release_uaf_repro(void **state_arg) {
  ...
  ASSERT_MEMORY_ALL_BALANCED({
    ...
    // global vars.
    Var *v1 = Var_create(Symbol_create(String_create("v1")));
    Ptr_retain(v1); // Retain so it survives bindRoot consumption
    Var_bindRoot(v1, s1);
    compState.registerVar("user/v1", v1);

    Var *v2 = Var_create(Symbol_create(String_create("v2")));
    Ptr_retain(v2);
    Var_bindRoot(v2, s2);
    ...
    printf("Executing JIT function (expecting double release crash if not "
           "fixed)...\n");
    bool thrown = false;
    try {
      func();
