...
namespace rt {
  TypedValue CodeGen::codegen(const Node &node, const ConstNode &subnode, const ObjectTypeSet &typeRestrictions) {
    auto types = getType(node, subnode, typeRestrictions);
    
    if (types.size() == 0) {
      throwCodeGenerationException(
                                   string("This type of constant cannot be inserted because of type mismatch:  ") + 
    ...
    if (!types.isDetermined()) {
      throwCodeGenerationException(
                                   string("Constant type must be well defined. Instead we have found a composite type: ") + 
                                   types.toString(), 
                                   node);
    }
    ...
    /* Heap objects need to have an extra retain generated into the code, because a new
       retain is needed every time the node is *executed* not compiled */    
    switch (types.determinedType()) {
      ...
      // case deftypeType:
      //   throwCodeGenerationException(
      //                                string("Not possible to create const of type: ") + 
      //                                ConstNode_ConstType_Name(subnode.type()), 
      //                                node);
      //   break;
      case keywordType:
        retVal = dynamicConstructor.createKeyword((name[0] == ':' ? name.substr(1) : name).c_str());
        break;
      case bigIntegerType:
      ...
      //     if (!var) {
      //       throwCodeGenerationException("Unable to resolve var: " + subnode.val() + " in this context", node);
      //     }
  ...
  ObjectTypeSet CodeGen::getType(const Node &node, const ConstNode &subnode,
    ...
    switch (subnode.type()) {
      case ConstNode_ConstType_constTypeNumber:
        ...
        throwCodeGenerationException(string("Compiler only supports 32 bit integers at this time. "), node);
        break;
