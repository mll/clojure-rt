...
namespace rt {
  ...
  void MemoryManagement::dynamicMemoryGuidance(const MemoryManagementGuidance &guidance) {
    ...
    for (word_t depth = variableBindingStack.stackDepth() - 1; depth >= 0;
      ...
      if (val) {
        while(change != 0) {
          if(change > 0) { dynamicRetain(*val); change--; }
          else { dynamicRelease(*val); change++; }
        }
        break;
      }
    }
  ...
  void MemoryManagement::dynamicRetain(TypedValue &target) {
     ...
     if (target.type.isDetermined()) {
       ...
       Value *gepPtr = builder.CreateStructGEP(types.RT_objectTy, prepare.value, 0, "get_refcount");
     ...
     TypedValue retain = invoke.invokeRuntime("retain", nullptr, {ObjectTypeSet::all()}, {valueEncoder.box(target)});     
  ...
  TypedValue MemoryManagement::dynamicRelease(TypedValue &target) {
     ...
     if (target.type.isDetermined()) {
       ...
       Value *condValue = builder.CreateICmpEQ(rmw, dynamicConstructor.createBoolean(true).value, "cmp_jj");
       Function *parentFunction = builder.GetInsertBlock()->getParent();
       BasicBlock *destroyBB = llvm::BasicBlock::Create(context, "destroy", parentFunction);  
       ...
       phiNode->addIncoming(dynamicConstructor.createBoolean(false).value, ignoreBB);
       return TypedValue(ObjectTypeSet(booleanType), phiNode);                  
     }     
#endif
     
     TypedValue release = invoke.invokeRuntime("release", nullptr, {ObjectTypeSet::all()}, {valueEncoder.box(target)});     
