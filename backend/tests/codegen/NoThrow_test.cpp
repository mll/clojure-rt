  ...
  ASSERT_MEMORY_ALL_BALANCED({
    ...
    for (int i = 0; i < 2; ++i) {
      ...
      for (int j = 0; j < 2; ++j) {
        ...
        c->set_op(opConst);
        c->mutable_subnode()->mutable_const_()->set_type(
...
static void test_hot_cold_separation(void **state) {
  ...
  ASSERT_MEMORY_ALL_BALANCED({
    ...
    compState.registerVar("x", Var_create(RT_boxPtr(nameX)));
    auto *nameY = String_createDynamicStr("y");
    compState.registerVar("y", Var_create(RT_boxPtr(nameY)));
...
static void test_whitelist_works(void **state) {
  ...
  ASSERT_MEMORY_ALL_BALANCED({
    ...
    Node root;
    root.set_op(opVar);
    root.mutable_subnode()->mutable_var()->set_var(
        "v/clojure.core/*print-length*");
