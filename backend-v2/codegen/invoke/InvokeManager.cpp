...
ObjectTypeSet InvokeManager::returnValueType(PersistentArrayMap *description) {
  RTValue returnValueTypeIndicator = PersistentArrayMap_get(
    ...
    if (returnValueTypeIndicatorType != keywordType &&
      ...
      throwInternalInconsistencyException("Return type must be an alias or a symbol");
    }

    String *compactifiedReturnTypeIndicatorString =
        String_compactify(toString(returnValueTypeIndicator));

    PersistentArrayMap *returnValueType = state.internalClassRegistry.getCurrent(String_c_str(compactifiedReturnTypeIndicatorString));
    Ptr_release(compactifiedReturnTypeIndicatorString);
    
    RTValue returnValueTypeEnum =
        PersistentArrayMap_get(returnValueType,
                               Keyword_create(String_create("object-type")));

    if (getType(returnValueTypeEnum) != integerType) {
      release(returnValueTypeEnum);
      throwInternalInconsistencyException("Return value type must be an internal type");
    }


    ObjectTypeSet returnValueTypeSet((objectType)RT_unboxInt32(returnValueTypeEnum));
...
bool InvokeManager::checkIntrinsicArgs(PersistentArrayMap *description,
  ...
  RTValue argsVecRaw =
    PersistentArrayMap_get(description, Keyword_create(String_create("args")));

  if (getType(argsVecRaw) != persistentVectorType) {
    release(argsVecRaw);
    throwInternalInconsistencyException("Intrinsic call: :args is not a vector");
  }
  
  PersistentVector *argsVec =
    (PersistentVector *)RT_unboxPtr(argsVecRaw);
  ...
  for (size_t i = 0; i < args.size(); i++) {
    ...
    /* Intrinsics are supported only for built-in objects (represented by ObjectTypeSet) */
    if (!argClass) {
      release(argsVecRaw);
      throwInternalInconsistencyException("Intrinsic call: only basic types can be used in the intrinsics");
    }
    Ptr_retain(argClass);
    RTValue className =
        PersistentArrayMap_get(argClass, Keyword_create(String_create("name")));
    RTValue classAlias =
        PersistentArrayMap_get(argClass, Keyword_create(String_create("alias")));
...
InvokeManager::generateIntrinsic(RTValue intrinsicDescription,
  ...
  try {
    ...
    String *compactifiedIntrinsicName = String_compactify((String *)RT_unboxPtr(intrinsicName));
    ...
    if (equals(callType, intriniscKeyword)) {
      ...
      if (block == intrinsics.end()) {
        ...
        release(callKeyword);
        throwInternalInconsistencyException("Intrinsic '" + stringIntrinsicName + "' does not exist.");
      }
      ...
      release(callKeyword);
      Ptr_release(description);
      return invokeRuntime(stringIntrinsicName, &returnValueTypeSet, argTypes, args);
    } else {
      release(intriniscKeyword);        
      release(callType);
