...
(defn -collect-closed-overs
  [ast]
  ;; (println "-collect-closed-overs" (:op ast))
  (-> (case (:op ast)
...
(defn collect-closed-overs*
  [{:keys [op] :as ast}]
  ;; (println "collect-closed-overs*" op)
    ...
    (if collect?
      (let [[ast {:keys [closed-overs locals]}]
            (binding [*collects* (atom (merge @*collects*
                                              {:closed-overs {} :locals #{}}))]
              [(-collect-closed-overs ast) @*collects*])]
        ;; (println "collect-closed-overs* IF-true" op closed-overs locals)
