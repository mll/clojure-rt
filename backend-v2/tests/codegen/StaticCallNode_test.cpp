...
static void setup_mock_runtime_full(rt::ThreadsafeCompilerState &compState) {
    // Numbers class with overloaded add: 2 specialized (int, double), 1 generic
    {
        ...
        addII.argTypes = { ObjectTypeSet(integerType, false), ObjectTypeSet(integerType, false) };
        ...
        addDD.argTypes = { ObjectTypeSet(doubleType, false), ObjectTypeSet(doubleType, false) };
        ...
        addGen.argTypes = { ObjectTypeSet::dynamicType(), ObjectTypeSet::dynamicType() };
        ...
        addBB.argTypes = { ObjectTypeSet(booleanType, false), ObjectTypeSet(booleanType, false) };
        ...
        addComplex.argTypes = { ObjectTypeSet(integerType, false), ObjectTypeSet(integerType, false) };
...
static void test_dynamic_dispatch_3tail(void **state) {
    ...
    // Test Case 1: Int + Int
    {
        ...
        auto resCall = engine.compileAST(callNode, "__test_3tail_int", llvm::OptimizationLevel::O0, true).get();
    ...
    // Test Case 2: Double + Double
    {
        ...
        auto resCallD = engine.compileAST(callNode, "__test_3tail_double", llvm::OptimizationLevel::O0, true).get();
...
static void test_dynamic_dispatch_filtering(void **state) {
    ...
    try {
        auto resCall = engine.compileAST(callNode, "__test_filtering", llvm::OptimizationLevel::O0, true).get();
        RTValue result = resPtrToValue(resCall);
...
static void test_dynamic_dispatch_exhaustive(void **state) {
    ...
    try {
        auto resCall = engine.compileAST(callNode, "__test_exhaustive", llvm::OptimizationLevel::O0, true).get();
        RTValue result = resPtrToValue(resCall);
        assert_true(RT_isInt32(result));
        assert_int_equal(1030, RT_unboxInt32(result));
        release(result);
...
static void test_dynamic_dispatch_no_match(void **state) {
    ...
    try {
        auto resCall = engine.compileAST(callNode, "__test_no_match", llvm::OptimizationLevel::O0, true).get();
        // Since both are strings, it's statically possible they match a dynamic overload if it existed, 
        // but we filtered versions. (String, String) is NOT statically possible for any of (int, int), (double, double), (bool, bool).
        // Wait, if it's NOT statically possible, codegen should throw compile-time error!
        assert_true(false); // Should not reach here
    } catch (const LanguageException &e) {
        // Correctly threw compile-time exception because no statically possible overloads exist
        assert_true(true);
    } catch (const std::exception &e) {
...
static void test_dynamic_dispatch_no_match_runtime(void **state) {
    ...
    // To test RUNTIME exception, we need arguments that are statically ANY but runtime NOT matching any specialization.
    Node callNode;
    callNode.set_op(opStaticCall);
    auto *sc = callNode.mutable_subnode()->mutable_staticcall();
    sc->set_class_("clojure.lang.Numbers");
    ...
    try {
        auto resCall = engine.compileAST(callNode, "__test_no_match_runtime", llvm::OptimizationLevel::O0, true).get();
...
static void test_regression_type_narrowing_specialized(void **state) {
    ...
    // This test specifically documents the fix for the "InternalInconsistencyException"
    // caused by type mismatch in specialized branches. 
    // We call "add" with arguments that are statically indeterminate (Any).
    // The dispatch logic will:
    // 1. Check if they are (int, int) at runtime.
    // 2. Unbox them if true.
    // 3. Call mock_add_int(i32, i32).
    // The InvokeManager::generateIntrinsic checks if the arguments passed to mock_add_int
    // match the IntrinsicDescription. If we don't narrow the type of the unboxed TypedValue,
    // it will still be "Any" and InvokeManager will throw an exception.
    Node callNode;
    callNode.set_op(opStaticCall);
    auto *sc = callNode.mutable_subnode()->mutable_staticcall();
    sc->set_class_("clojure.lang.Numbers");
    ...
    try {
        auto resCall = engine.compileAST(callNode, "__test_regression_narrowing", llvm::OptimizationLevel::O0, true).get();
        RTValue result = resPtrToValue(resCall);
        assert_true(RT_isInt32(result));
...
static void test_regression_phi_node_segfault(void **state) {
    ...
    // This test specifically documents the fix for the LLVM segfault (SimplifyCFG crash)
    // caused by malformed PHI nodes when a specialized branch contains internal branching.
    // "complex_add" specialization "Add" (intrinsic) creates "overflow" and "no_overflow" blocks.
    Node callNode;
    callNode.set_op(opStaticCall);
    auto *sc = callNode.mutable_subnode()->mutable_staticcall();
    sc->set_class_("clojure.lang.Numbers");
    ...
    try {
        // Optimization level O1 or higher is needed to trigger SimplifyCFG in a way that crashes
        auto resCall = engine.compileAST(callNode, "__test_regression_segfault", llvm::OptimizationLevel::O1, true).get();
        RTValue result = resPtrToValue(resCall);
        assert_true(RT_isInt32(result));
