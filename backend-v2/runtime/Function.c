...
/* mem done */
ClojureFunction *Function_create(uword_t methodCount, uword_t maxArity,
                                 bool once) {
...
/* outside refcount system (w.r.t. self) and closedOvers (they need to be
 * retained already when this fun is called) */
void Function_fillMethod(ClojureFunction *self, uword_t position, uword_t index,
                         uword_t fixedArity, bool isVariadic,
  ...
  if (closedOversCount > 0)
    method->closedOvers = allocate(closedOversCount * sizeof(RTValue));
  va_list args;
...
bool Function_validCallWithArgCount(ClojureFunction *self, uword_t argCount) {
  if (argCount <= self->maxArity) {
    for (uword_t i = 0; i < self->methodCount; ++i) {
      if (self->methods[i].fixedArity == argCount &&
          !self->methods[i].isVariadic) {
        return true;
      }
    }
  } else {
...
String *Function_toString(ClojureFunction *self) {
  ...
  for (uword_t i = 0; i < self->methodCount; ++i) {
    ...
    pos += snprintf(buf + pos, sizeof(buf) - pos,
                    "%s[arity %d, %s, closed-overs %d, impl %p]",
                    (i > 0 ? ", " : ""), (int)m->fixedArity,
                    (m->isVariadic ? "var" : "fixed"), (int)m->closedOversCount,
                    m->baselineImplementation);
...
void Function_cleanupOnce(ClojureFunction *self) {
  ...
  for (uword_t i = 0; i < self->methodCount; i++) {
    ...
    for (uword_t j = 0; j < method->closedOversCount; j++)
      release(method->closedOvers[j]);
    if (method->closedOversCount)
...
FunctionMethod *Function_extractMethod(RTValue funObj, uword_t argCount) {
  ...
  for (uword_t i = 0; i < self->methodCount; ++i) {
    if (!self->methods[i].isVariadic &&
        self->methods[i].fixedArity == argCount) {
  ...
  for (uword_t i = 0; i < self->methodCount; ++i) {
    if (self->methods[i].isVariadic &&
        argCount >= self->methods[i].fixedArity) {
      return &self->methods[i];
    }
  }
...
RTValue RT_packVariadic(uword_t argCount, RTValue *args, uword_t fixedArity) {
  if (argCount <= fixedArity)
    return RT_boxNil();
  return RT_createListFromArray((int32_t)(argCount - fixedArity),
                                args + fixedArity);
}
...
struct FunctionMethod Global_Keyword_Method_1 = {.fixedArity = 1,
                                                 .isVariadic = false,
                                                 .baselineImplementation =
                                                     Baseline_Keyword_Invoke_1};
struct FunctionMethod Global_Keyword_Method_2 = {.fixedArity = 2,
                                                 .isVariadic = false,
                                                 .baselineImplementation =
                                                     Baseline_Keyword_Invoke_2};
struct FunctionMethod Global_Map_Method_1 = {.fixedArity = 1,
                                             .isVariadic = false,
                                             .baselineImplementation =
                                                 Baseline_Map_Invoke_1};
struct FunctionMethod Global_Vector_Method_1 = {.fixedArity = 1,
                                                .isVariadic = false,
                                                .baselineImplementation =
                                                    Baseline_Vector_Invoke_1};
...
struct FunctionMethod *RT_updateICSlot(void *slot, RTValue currentVal,
  ...
  if (type == functionType) {
    ...
    if (argCount == 1)
      method = &Global_Keyword_Method_1;
    else if (argCount == 2)
      method = &Global_Keyword_Method_2;
  } else if (type == persistentArrayMapType) {
    if (argCount == 1)
      method = &Global_Map_Method_1;
  } else if (type == persistentVectorType) {
    if (argCount == 1)
      method = &Global_Vector_Method_1;
  }

  if (!method) {
    throwIllegalStateException_C(
        "Attempted to invoke non-callable value or invalid arity");
  }
