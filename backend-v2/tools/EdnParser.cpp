...
namespace rt {  
  ~ClassDescription::ClassDescription() {
    for (auto s : staticFields) {
      release(s->second);
    }    
  }

  TemporaryClassData::~TemporaryClassData() {
    for (auto d : classesByName) {
      Ptr_release(d->second);
    }
    for (auto d : classesById) {
      Ptr_release(d->second);
    }          
  }
  ...
  TemporaryClassData::TemporaryClassData(RTValue from) {
    ...
    for (uword_t i = 0; i < map->count; i++) {
      ...
      if (getType(classId) == integerType) {
        retain(value);
        classesById[RT_unboxInt32(classId)] = (PersistentArrayMap *)RT_unboxPtr(value) 
      } else {
      ...
      if (getType(alias) == keywordType) {
        ...
        retain(value);
        classesByName[string(String_c_str(ss))] = (PersistentArrayMap *)RT_unboxPtr(value)
        Ptr_release(ss);
      } else {
      ...
      retain(value);
      classesByName[string(String_c_str(s))] = (PersistentArrayMap *)RT_unboxPtr(value);
      Ptr_release(s);
    }    
    release(from);
  }
  ...
  ClassDescription::ClassDescription(RTValue from,
    ...
    if (getType(from) != persistentArrayMapType) {
      release(from);
      throwInternalInconsistencyException("ClassDescription: requires a map as an argument");
    }    
    PersistentArrayMap *description =
      (PersistentArrayMap *)RT_unboxPtr(from);
  ...
  ClassDescription::parseStaticFields(RTValue from) {
    ...
    for (uword_t i = 0; i < map->count; i++) {
      ...
      if (getType(key) != symbolType)
        throwInternalInconsistencyException(
                                            "Static fields key is not a symbol.");      
      retain(key);
      retain(value);
  ...
  ClassDescription::parseIntrinsics(RTValue from, TemporaryClassData &classData) {
    ...
    for (uword_t i = 0; i < map->count; i++) {
      ...
      if (descriptionsType != nilType) {
        if (descriptionsType != persistentVector) {
          release(from);
          throwInternalInconsistencyException("Intrinsic collection is not a map");
        }
        ...
        PersistentVectorIterator it = PersistentVector_iterator(descriptionsVector);
