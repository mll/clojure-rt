...
static JITEngine *engine = nullptr;
static ThreadsafeCompilerState *compState = nullptr;

static int group_setup(void **state) {
    compState = new ThreadsafeCompilerState();
    setup_test_metadata(*compState);
    engine = new JITEngine(*compState);
    return 0;
}
...
static void test_is_instance_compile_time_true(void **state) {
  ...
  clsNode->mutable_subnode()->mutable_const_()->set_type(ConstNode_ConstType_constTypeClass);
  clsNode->mutable_subnode()->mutable_const_()->set_val("MyClass");

  auto res = engine->compileAST(node, "test_is_instance_compile_time_true",
                               llvm::OptimizationLevel::O0, true).get().address;
...
static void test_is_instance_runtime(void **state) {
  ...
  init->set_op(opConst);
  init->mutable_subnode()->mutable_const_()->set_type(ConstNode_ConstType_constTypeNil);
  ln->mutable_body()->CopyFrom(node);

  auto res = engine->compileAST(letNode, "test_is_instance_runtime",
                               llvm::OptimizationLevel::O0, true).get().address;
...
static void test_is_instance_class_prefix(void **state) {
  ...
  auto res = engine->compileAST(node, "test_is_instance_class_prefix",
                               llvm::OptimizationLevel::O0, true).get().address;
...
static void test_is_instance_any_type_regression(void **state) {
  ...
  auto res = engine->compileAST(letNode, "test_is_instance_any_type_regression",
                               llvm::OptimizationLevel::O0, true).get().address;
...
static void test_is_instance_refcounted_local(void **state) {
  ...
  ASSERT_MEMORY_ALL_BALANCED({
    ...
    nn->mutable_class_()->mutable_subnode()->mutable_const_()->set_val("MyClass");
    ...
    ret->set_op(opConst);
    ret->mutable_subnode()->mutable_const_()->set_type(ConstNode_ConstType_constTypeNil);
...
int main(void) {
  ...
  const struct CMUnitTest tests[] = {
      cmocka_unit_test_setup_teardown(test_is_instance_compile_time_true, group_setup, group_teardown),
      cmocka_unit_test_setup_teardown(test_is_instance_compile_time_false, group_setup, group_teardown),
      cmocka_unit_test_setup_teardown(test_is_instance_runtime, group_setup, group_teardown),
      cmocka_unit_test_setup_teardown(test_is_instance_class_prefix, group_setup, group_teardown),
      cmocka_unit_test_setup_teardown(test_is_instance_any_type_regression, group_setup, group_teardown),
      cmocka_unit_test_setup_teardown(test_is_instance_refcounted_local, group_setup, group_teardown),
  };
