...
I. Create NewType.h and NewType.c files in backend/runtime (You can base on String.h/c) and add .c file in runtime/CMakeLists.txt
...
III. Add the desired types enum to ObjectProto.h (add it before last type, which should remain persistentArrayMapType)

IV. Edit runtime/Object.h and add appropriate clauses to 
   a) Object_destroy 
   b) Object_hash
   c) Object_equals
   d) Object_toString
