 ...
 * key.
 * 4. Returns the bridge address.
 */

// Slow path has simple semantics: it NEVER consumes arguments. This ensures that
// the caller (JIT or C++ test) can safely reuse or release them.
...
#pragma clang diagnostic pop
  ...
  uword_t currentEpochValue =
      atomic_load_explicit(&globalMethodICEpoch, memory_order_acquire);
  word_t expectedKey =
      (word_t)((currentEpochValue << 32) | (word_t)instanceType);

  if (currentIC.key == expectedKey) {
    return currentIC.value;
  }

  InlineCache newCache = {expectedKey, bridgePtr};
  if constexpr (K_WORD_SIZE == 8) {
#if defined(__clang__)
#pragma clang diagnostic push
#pragma clang diagnostic ignored "-Watomic-alignment"
