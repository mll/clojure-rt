...
public:
  ...
  /// Allocates the exception slot and creates the terminal resume block for unwinding.
  /// This is called directly for flat, independent functions (like topLevel or
  /// inlinecachecall), or internally via suspendStateForNestedFunction for nested clojurefunctions.
  ...
  /// The landing pad routes execution through the cleanup stack to release all
  /// currently active resources. 
  /// @param skipCount Number of top active resources to exclude from cleanup (useful
  ///                  when the throwing expression shouldn't release its own result).
  /// @param extraCleanup Additional ad-hoc resources to release during this specific unwind.
  llvm::BasicBlock *
  ...
  /// Checks if there is any active AST-provided unwind guidance (instructions for
  /// manually modifying reference counts during unwind) currently in effect.
  bool hasActiveGuidance() const;
  
  /// Resets the memory management state, clearing all resources, caches, and guidance.
  void clear();

  /// Encapsulates the memory management state for a single function being compiled.
  /// Used to preserve state when pausing compilation of one function to compile another
  /// (e.g., when generating IR for nested closures or lambdas).
  ...
  /// Saves the current memory management state onto the stateStack and initializes a
  /// fresh state for F by calling initFunction(F). This is used when pausing the
  /// compilation of an outer function to compile a nested clojurefunction (e.g., a lambda),
  /// ensuring the outer function's active resources and landing pads are preserved.
  ...
  /// Called after a nested clojurefunction finishes compilation, allowing the compiler
  /// to resume generating the outer function with its original resources intact.
  ...
  /// Sets the AST-provided memory management guidance to be applied during unwinding.
  /// This also clears the landing pad cache since the generated landing pads will
  /// now need to include the new guidance operations.
  ...
  /// A RAII guard for temporarily applying AST-provided memory management guidance
  /// during the compilation of a specific expression or block, restoring the previous
  /// guidance when it goes out of scope.
...
private:
  ...
  /// An alloca instruction in the function's entry block used to temporarily store
  /// the caught exception pointer during cleanup block execution.
  llvm::Value *exceptionSlot = nullptr;
  
  /// The final basic block in the unwind path that executes the 'resume' instruction
  /// to continue propagating the exception up the call stack after all local cleanups.
  llvm::BasicBlock *terminalResumeBB = nullptr;
  
  /// A stack of basic blocks, where each block releases one or more active resources
  /// and then branches to the next block in the stack or to the terminalResumeBB.
  std::vector<llvm::BasicBlock *> cleanupStack;
  ...
  /// The total number of resources pushed over the lifetime of the current function state.
  /// Used to uniquely identify the current unwind state for landing pad caching.
  size_t totalPushedResources = 0;
  
  /// An index into `activeResources` representing the high-water mark for which
  /// cleanup blocks have already been generated. Prevents redundant cleanup generation.
  size_t resourcesWithCleanup = 0; // Index into activeResources
  
  /// A cache mapping an unwind state identifier (based on totalPushedResources and skipCount)
  /// to its corresponding landing pad block. Avoids emitting identical landing pads multiple times.
  std::map<size_t, llvm::BasicBlock *> lpadCache;
