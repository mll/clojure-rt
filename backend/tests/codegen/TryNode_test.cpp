...
static void prepareExceptionClass(uint32_t type,
...
static void test_try_catch_simple(void **state) {
  ...
  LanguageException *lePtr =
      new LanguageException("CodeGenerationException", msg, RT_boxNil());
  ...
  prepareExceptionClass(exceptionType, threadsafeState,
  ...
  // We expect this to throw an "CodeGenerationException" (because string is
  // thrown), be caught by our catch clause, and return the exception object
  // itself.
...
static void test_try_catch_nested_throw(void **state) {
  ...
  LanguageException *lePtrA =
      new LanguageException("CodeGenerationException", msgA, RT_boxNil());
  ...
  LanguageException *lePtrB =
      new LanguageException("CodeGenerationException", msgB, RT_boxNil());
  ...
  th->mutable_exception()->mutable_subnode()->mutable_var()->set_var(
      "#'my_exception_var_A");
  ...
  catchTh->mutable_exception()->mutable_subnode()->mutable_var()->set_var(
      "#'my_exception_var_B");

  auto resThrow =
      engine.compileAST(tryNode, "__test_try_catch_nested_throw").get();
...
static void test_try_finally_unwind(void **state) {
  ...
  th->mutable_exception()->set_op(opVar);
  th->mutable_exception()->mutable_subnode()->mutable_var()->set_var(
      "#'my_exception_var_unwind");

  // finally body just returns a constant 42 (which is ignored because unwind
  // continues)
  auto *finallyNode = tr->mutable_finally();
  finallyNode->set_op(opConst);
  finallyNode->mutable_subnode()->mutable_const_()->set_type(
