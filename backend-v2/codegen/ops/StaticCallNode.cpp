...
TypedValue CodeGen::codegen(const Node &node, const StaticCallNode &subnode,
  ...
  if (allDetermined) {
    ...
    for (const auto &id : versions) {
      if (id.argTypes.size() == args.size()) {
        ...
        cout << "Checking match: " << id.symbol << endl;
  ...
  if (bestMatch) {
    cout << "Best match: " << bestMatch->symbol << endl;
    ...
    cout << "BestMatchReturnType: " << bestMatch->returnType.toString() << endl;
    return result;
  }
  ...
  // If types are determined but NO match found and NO generic fallback -> Fail
  // early
